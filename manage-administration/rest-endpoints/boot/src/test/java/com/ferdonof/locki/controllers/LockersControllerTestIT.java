...
class LockersControllerTestIT {
  ...
  private RackEntity createRack() {
    return this.rackRepository.saveAndFlush(RackEntity
        .builder()
        .serial(1)
  ...
  private LockerEntity createLocker(RackEntity rack) {
    return this.lockerRepository.saveAndFlush(LockerEntity
        .builder()
        .serial(10)
        .rack(rack)
  ...
  void create_whenValidRequest_shouldReturn201() throws Exception {
    ...
    final LockerEntity persisted = this.lockerRepository
        .findAll()
        .getFirst();
    assertThat(persisted.getSerial()).isEqualTo(10);
    assertThat(persisted.getRack()).isNotNull();
    assertThat(persisted
        .getRack()
        .getId()).isEqualTo(savedRack.getId());
  ...
  void getById_whenLockerExists_shouldReturn200() throws Exception {
    ...
    this.mockMvc
        .perform(get(LOCKERS_URL + "/{id}", locker.getId()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(locker
            .getId()
            .toString()))
        .andExpect(jsonPath("$.number").value(10))
        .andExpect(jsonPath("$.status").value("AVAILABLE"))
  ...
  void getById_whenLockerNotExists_shouldReturn404() throws Exception {
    ...
    this.mockMvc
        .perform(get(LOCKERS_URL + "/{id}", randomId))
  ...
  void update_whenValidRequest_shouldReturn200() throws Exception {
    ...
    this.mockMvc
        .perform(put(LOCKERS_URL + "/{id}", locker.getId())
            .contentType(MediaType.APPLICATION_JSON)
            .content(requestBody))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(locker
    ...
    final LockerEntity updated = this.lockerRepository
        .findById(locker.getId())
        .orElseThrow();
    assertThat(updated.getStatus()).isEqualTo(LockerStatus.MAINTENANCE);
    assertThat(updated.getLatchStatus()).isEqualTo(LOCKED);
  }
  ...
  void update_whenLockerNotExists_shouldReturn404() throws Exception {
    ...
    this.mockMvc
        .perform(put(LOCKERS_URL + "/{id}", randomId)
            .contentType(MediaType.APPLICATION_JSON)
            .content(requestBody))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code").value(404))
  ...
  void update_whenMissingRequiredFields_shouldReturn400() throws Exception {
    ...
    this.mockMvc
        .perform(
            put(LOCKERS_URL + "/{id}", locker.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content("{}"))
        .andExpect(status().isBadRequest());
  }
  ...
  void search_whenLockersExist_shouldReturnList() throws Exception {
    ...
    this.lockerRepository.saveAndFlush(LockerEntity
        .builder()
        .serial(20)
        .rack(rack)
    ...
    this.mockMvc
        .perform(post(LOCKERS_URL + "/search")
            .contentType(MediaType.APPLICATION_JSON)
            .content("""
                { "limit": 10, "offset": 0 }
                """))
  ...
  void changeStatus_whenLockerExists_shouldReturn200() throws Exception {
    ...
    this.mockMvc
        .perform(patch(LOCKERS_URL + "/{id}/status/{status}", locker.getId(), "MAINTENANCE"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(locker
            .getId()
            .toString()))
  ...
  void changeStatus_whenLockerNotExists_shouldReturn404() throws Exception {
    ...
    this.mockMvc
        .perform(patch(LOCKERS_URL + "/{id}/status/{status}", randomId, "MAINTENANCE"))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code").value(404))
        .andExpect(jsonPath("$.title").value("Not Found"))
        .andExpect(jsonPath("$.detail").value("Locker with id '%s' not found".formatted(randomId)));
  ...
  void changeStatus_whenInvalidStatus_shouldReturn400() throws Exception {
    ...
    this.mockMvc
        .perform(patch(LOCKERS_URL + "/{id}/status/{status}", locker.getId(), "INVALID_STATUS"))
        .andExpect(status().isBadRequest());
  }
  ...
  void changeLatchStatus_whenLockerExists_shouldReturn200() throws Exception {
    ...
    this.mockMvc
        .perform(patch(LOCKERS_URL + "/{id}/latch/status/{status}", locker.getId(), "OPEN"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(locker
            .getId()
            .toString()))
        .andExpect(jsonPath("$.latchStatus").value("OPEN"))
        .andExpect(jsonPath("$.status").value("AVAILABLE"));

    final LockerEntity updated = this.lockerRepository
        .findById(locker.getId())
        .orElseThrow();
    assertThat(updated.getLatchStatus()).isEqualTo(LatchStatus.UNLOCKED);
  }
  ...
  void changeLatchStatus_whenLockerNotExists_shouldReturn404() throws Exception {
    ...
    this.mockMvc
        .perform(patch(LOCKERS_URL + "/{id}/latch/status/{status}", randomId, "OPEN"))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code").value(404))
        .andExpect(jsonPath("$.title").value("Not Found"))
        .andExpect(jsonPath("$.detail").value("Locker with id '%s' not found".formatted(randomId)));
  ...
  void changeLatchStatus_whenInvalidStatus_shouldReturn400() throws Exception {
    ...
    this.mockMvc
        .perform(patch(LOCKERS_URL + "/{id}/latch/status/{status}", locker.getId(), "INVALID_LATCH"))
        .andExpect(status().isBadRequest());
  }
}
