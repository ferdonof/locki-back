...
class RacksControllerTestIT {
  ...
  private LocationEntity createLocation() {
    return this.locationRepository.saveAndFlush(LocationEntity
        .builder()
        .code("MAD")
        .city("Madrid")
        .country("Spain")
        .lat(BigDecimal.valueOf(40.4168))
        .lon(BigDecimal.valueOf(-3.7038))
        .build());
  }
  ...
  void getById_whenRackExists_shouldReturn200() throws Exception {
    ...
    this.mockMvc
        ...
        .andExpect(jsonPath("$.id").value(rack
            .getId()
            .toString()))
        .andExpect(jsonPath("$.number").value(1))
        .andExpect(jsonPath("$.status").value("ACTIVE"))
  ...
  void update_whenValidRequest_shouldReturn200() throws Exception {
    ...
    final RackEntity updated = this.rackRepository
        .findById(rack.getId())
        .orElseThrow();
  ...
  void search_whenRacksExist_shouldReturnList() throws Exception {
    ...
    this.rackRepository.saveAndFlush(RackEntity
        .builder()
        .serial(2)
