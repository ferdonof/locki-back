...
class CreateLocationTest {
	...
	void execute_whenValidLocation_shouldReturnSavedLocation() {
		final Location input = Location.builder().code("MAD").city("Madrid").country("Spain")
				.latitude(new BigDecimal("40.416775")).longitude(new BigDecimal("-3.703790")).build();
		final Location expected = Location.builder().id(UUID.randomUUID()).code("MAD").city("Madrid").country("Spain")
				.latitude(new BigDecimal("40.416775")).longitude(new BigDecimal("-3.703790")).version(0L)
