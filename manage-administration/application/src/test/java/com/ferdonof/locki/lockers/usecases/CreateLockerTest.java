...
class CreateLockerTest {
  ...
  void execute_whenRackExists_shouldCreateLockerWithRack() {
    final Rack rack = Rack
        .builder()
        .id(RACK_ID)
        .serial(1)
        .build();
    final CreateLockerRequest request = CreateLockerRequest
        .builder()
        .serial(LOCKER_NUMBER)
        .rackId(RACK_ID)
    ...
    assertThat(capturedLocker.serial()).isEqualTo(LOCKER_NUMBER);
    assertThat(capturedLocker.rack()).isEqualTo(rack);
    assertThat(capturedLocker.status()).isEqualTo(LockerStatus.AVAILABLE);
    assertThat(capturedLocker.latchStatus()).isEqualTo(LOCKED);
  }
  ...
  void execute_whenValidLockerWithRack_shouldCreateLocker() {
    ...
    final var request = CreateLockerRequest
        .builder()
        .serial(10)
        .rackId(rackId)
        .status(AVAILABLE)
    ...
    final var createdLocker = Locker
        .builder()
        .id(lockerId)
        .serial(10)
        .rack(rack)
